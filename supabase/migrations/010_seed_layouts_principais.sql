...
BEGIN
  ...
  -- Encoding: UTF-8 com BOM (stripped automaticamente)
  ...
  --  7  Nº Inscrição            → referencia
  --  8  Descrição Produto       → produto
  -- 10  Valor base              → valor_base
  -- 11  Parcela comissionada    → parcela_comissionada
  -- 12  Competência comissionada→ data_competencia  (YYYYMM)
  ...
  IF v_seg_id IS NOT NULL THEN
    ...
    IF v_lay_id IS NOT NULL THEN
      ...
      VALUES
        (v_lay_id, 'referencia',           '7',  NULL),
        (v_lay_id, 'nome_segurado',        '3',  NULL),
        (v_lay_id, 'cpf_segurado',         '4',  NULL),
        (v_lay_id, 'data_competencia',     '12', 'YYYYMM'),
        (v_lay_id, 'produto',              '8',  NULL),
  ...
  -- 10  Vencimento   → data_competencia  (DD/MM/YYYY)
