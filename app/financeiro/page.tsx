...
export default async function FinanceiroPage({
  ...
  if (corretoraId) {
    const [segRes, grupoRes, prodRes, parcRes] = await Promise.all([
      db.from('seguradoras').select('id, nome_fantasia, nome').eq('status', 'ativo').order('nome'),
      db.from('grupos_produto').select('id, nome').order('nome'),
      db.from('produtos').select('id, nome, grupo_produto_id').order('nome'),
      db.from('parceiros').select('id, nome, pct_indicador, pct_corretor1, pct_corretor2').eq('tenant_id', corretoraId).eq('status', 'ativo').order('nome'),
    ])
  ...
  return (
    <AppLayout session={session} nav={nav} breadcrumb={[{ label: 'Apuração Mensal' }]}>
